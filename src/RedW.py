...
# Dicha red U se determina en 'OverlapSegmentationNet.py'.
...
from src import OverlapSegmentationNet
import torchvision.transforms as trans


class RedW(nn.Module):
    #%% 
    ...
    def __init__(self):
        ...
        super(RedW, self).__init__()
        #transformacion
        self.transform= trans.Normalize(mean=[0.5],
                                 std=[0.5])
        
        #Anterior usado para separacion
        self.layer0= OverlapSegmentationNet.OverlapSegmentationNet(canalesEntrada=1)
        
        #Se agrega softmax para pasar a probabilidades
        self.softmax2d= nn.Softmax2d()
        
        #Capas propias similares a OverlapSegmentationNet pero con 24 canales de entrada en la capa 1.
        #self.layer1= OverlapSegmentationNet.OverlapSegmentationNet(canalesEntrada=24)
        inplace= True
        padding = 1
        #Bloque 1
        self.layer1 = nn.Sequential(
                nn.Conv2d(24, 64, kernel_size=3, stride=1, padding=padding),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace),
    ...
    def forward(self, x):
        #aplicar transformacion que lo lleva a media 0.5 y std 0.5
        for i in range(x.size(0)):
            x[i] = self.transform(x[i])
        
        out1= self.layer0(x)
        out1= self.softmax2d(out1)
